...
def get_exp_learning_rate_scheduler(
    initial_learning_rate: float, decay_rate: float = 0.95
):
    return LearningRateScheduler(
        lambda epoch: initial_learning_rate * decay_rate ** epoch, verbose=True
    )


@scheduler_factory.register("cosine_scheduler")
def get_cosine_decay_scheduler(
    initial_learning_rate: float, decay_steps, alpha: float = 0.0
):
    return LearningRateScheduler(
        CosineDecay(initial_learning_rate, decay_steps, alpha=alpha,), verbose=True
    )
...
def get_piecewise_scheduler_with_warmups(
    ...
    def schedule_learning_rate(epoch):
        if epoch < lr_ramp_ep:
            ...
            ) / lr_ramp_ep * epoch + initial_learning_rate
